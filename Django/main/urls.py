...
from main import vancouver_house_v, user_v, system_intro_v
...
urlpatterns = [
    ...
    path(r'vancouver-house/list', vancouver_house_v.vancouver_house_list),
    path(r'vancouver-house/page', vancouver_house_v.vancouver_house_page),
    path(r'vancouver-house/save', vancouver_house_v.vancouver_house_save),
    path(r'vancouver-house/add', vancouver_house_v.vancouver_house_add),
    path(r'vancouver-house/info/<id_>', vancouver_house_v.vancouver_house_info),
    path(r'vancouver-house/detail/<int:id_>', vancouver_house_v.vancouver_house_detail),
    path(r'vancouver-house/update', vancouver_house_v.vancouver_house_update),
    path(r'vancouver-house/delete', vancouver_house_v.vancouver_house_delete),
    path(r'vancouver-house/stats', vancouver_house_v.vancouver_house_stats),
    path(r'vancouver-house/search', vancouver_house_v.vancouver_house_search),

    path(r'market-stats/list', vancouver_house_v.market_stats_list),
    path(r'market-stats/summary', vancouver_house_v.market_stats_summary),
    path(r'market-stats/trends', vancouver_house_v.market_stats_trends),

    path(r'system-info/details', system_intro_v.system_info),
    path(r'system-info/update', system_intro_v.system_update),
    path(r'system-info/stats', system_intro_v.system_stats),
]
# main app的路径
...
for i in os.listdir(mainDir):
    if i not in excludeList and i[-5:] == "_v.py":
        ...
        if tableName.lower() == "xinfangxinxi":
            urlpatterns.extend(
                [
                    path(r'{}/count'.format(tableName.lower()),
                         eval("{}_v.{}_count".format(tableName.capitalize(), tableName.lower()))),
...
urlpatterns.extend(
    [
        path(r'cal/<str:tableName>/<str:columnName>', schema_v.schema_cal),
        path(r'file/download', schema_v.schema_file_download),
        path(r'file/upload', schema_v.schema_file_upload),
        path(r'follow/<tableName>/<columnName>/<level>/<parent>', schema_v.schema_follow_level),
        path(r'follow/<tableName>/<columnName>', schema_v.schema_follow),
        path(r'location', schema_v.schema_location),
        path(r'matchFace', schema_v.schema_matchface),
        path(r'option/<tableName>/<columnName>', schema_v.schema_option),
        path(r'remind/<tableName>/<columnName>/<type>', schema_v.schema_remind_tablename_columnname_type),
        # 前台提醒接口（通用接口，不需要登陆）
        path(r'<tableName>/remind/<columnName>/<type>', schema_v.schema_tablename_remind_columnname_type),
        # 后台提醒接口 (每个表的单独接口，需登录)
        path(r'sh/<tableName>', schema_v.schema_sh),
        path(r'upload/<fileName>', schema_v.schema_upload),
        path(r'group/<tableName>/<columnName>', schema_v.schema_group_quyu),
        path(r'value/<tableName>/<xColumnName>/<yColumnName>', schema_v.schema_value_quyu),
        path(r'value/<tableName>/<xColumnName>/<yColumnName>/<timeStatType>', schema_v.schema_value_riqitj),
        path(r'spider/<tableName>', schema_v.schema_spider),
    ]
)
